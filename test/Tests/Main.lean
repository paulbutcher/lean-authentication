...
def main : IO UInt32 := do
  let checks := Tests.Flow.checks ++ (← Tests.Migrations.checks) ++ (← Tests.Sqlite.conformanceChecks)
    ++ (← Tests.EndToEnd.checks) ++ Tests.Sql.checks ++ (← Tests.Sql.poolChecks)
    ++ Tests.Template.checks ++ (← Tests.Signup.checks)
    ...
    ++ (← Tests.Http.emailedCodeChecks) ++ (← Tests.Http.rotationChecks)
    ++ Tests.Http.paramChecks ++ (← Tests.Webhooks.snsChecks)
    ...
    ++ (← Tests.FederatedSignIn.linkChecks)
    ++ (← Tests.FederatedFlow.checks) ++ (← Tests.FederatedFlow.invitationChecks) ++ (← Tests.FederatedFlow.limitChecks) ++ (← Tests.FederatedFlow.linkFlowChecks)
